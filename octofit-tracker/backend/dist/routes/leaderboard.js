...
router.get('/team/:teamId', async (req, res) => {
    try {
        const leaderboard = await Leaderboard_1.Leaderboard.find({ teamId: req.params.teamId })
            .sort({ score: -1 })
            .populate('userId', '-password')
            .populate('teamId');
        res.json(leaderboard);
...
router.get('/user/:userId', async (req, res) => {
    try {
        const entry = await Leaderboard_1.Leaderboard.findOne({ userId: req.params.userId })
            .populate('userId', '-password')
            .populate('teamId');
        if (!entry) {
